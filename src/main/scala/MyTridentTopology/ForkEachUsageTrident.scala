...
object ForkEachUsageTrident {
  def main (args: Array[String]) {
    val top = new TridentTopology

    val spoutStream = top.newStream("spout", new IntTupleSpout)

    spoutStream.each(new Fields("b"), new AddFunction, new Fields("d"))
      .each(new Fields("d"), new PrintFunction, new Fields())

    spoutStream.each(new Fields("a", "b", "c"), new PrintFunction, new Fields())
